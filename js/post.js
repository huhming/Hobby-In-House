$(document).ready(function () {
  ...
  $("#reply_save").click(function () {
    ...
    var reply =
      ...
      "   </td>" +
      '   <td width="100px">' +
      '       <input type="password" id="reply_password_' +
      reply_id +
      '" style="width:100px;" maxlength="10" placeholder="패스워드"/>' +
      "   </td>" +
      '   <td align="center">' +
      '       <button name="reply_reply" reply_id = "' +
      reply_id +
      '">댓글</button>' +
      '       <button name="reply_modify" r_type = "main" reply_id = "' +
      reply_id +
      '">수정</button>      ' +
      '       <button name="reply_del" reply_id = "' +
      reply_id +
      '">삭제</button>      ' +
      "   </td>" +
      "</tr>";
  ...
  $(document).on("click", "button[name='reply_modify']", function () {
    ...
    if (check) {
      ...
      var replyEditor =
        ...
        '   <td width="820px">' +
        '       <textarea name="reply_modify_content_' +
        reply_id +
        '" id="reply_modify_content_' +
        reply_id +
        '" rows="3" cols="50">' +
        txt_reply_content +
        "</textarea>" + //기존 내용 넣기
        "   </td>" +
        '   <td width="100px">' +
        '       <input type="text" name="reply_modify_writer_' +
        reply_id +
        '" id="reply_modify_writer_' +
        reply_id +
        '" style="width:100%;" maxlength="10" placeholder="작성자" value="' +
        txt_reply_writer +
        '"/>' + //기존 작성자 넣기
        "   </td>" +
        '   <td width="100px">' +
        '       <input type="password" name="reply_modify_password_' +
        reply_id +
        '" id="reply_modify_password_' +
        reply_id +
        '" style="width:100%;" maxlength="10" placeholder="패스워드"/>' +
        "   </td>" +
        '   <td align="center">' +
        '       <button name="reply_modify_save" r_type = "' +
        r_type +
        '" reply_id="' +
        reply_id +
        '">등록</button>' +
        '       <button name="reply_modify_cancel" r_type = "' +
        r_type +
        '" r_content = "' +
        txt_reply_content +
        '" r_writer = "' +
  ...
  $(document).on("click", "button[name='reply_modify_cancel']", function () {
    ...
    if (r_type == "main") {
      ...
      reply =
        ...
        "   </td>" +
        '   <td align="center">' +
        '       <button name="reply_modify" r_type = "sub" reply_id = "' +
        reply_id +
        '">수정</button>' +
        '       <button name="reply_del" reply_id = "' +
        reply_id +
  ...
  $(document).on("click", "button[name='reply_modify_save']", function () {
    ...
    if (
      $("#reply_modify_password_" + reply_id)
        .val()
        .trim() == ""
    ) {
      alert("패스워드를 입력하세요.");
    ...
    if (
      $("#reply_modify_content_" + reply_id)
        .val()
        .trim() == ""
    ) {
      alert("내용을 입력하세요.");
    ...
    var reply_content = $("#reply_modify_content_" + reply_id)
      .val()
      .replace("\n", "<br>"); //개행처리
  ...
  $(document).on("click", "button[name='reply_reply']", function () {
    ...
    var replyEditor =
      ...
      "   </td>" +
      '   <td align="center">' +
      '       <button name="reply_reply_save" reply_id="' +
      reply_id +
      '">등록</button>' +
    ...
    if (last_check) {
      //마지막이라면 제일 마지막 tr 뒤에 댓글 입력을 붙인다.
  ...
  $(document).on("click", "button[name='reply_reply_save']", function () {
    ...
    var reply_reply_content_val = reply_reply_content
      .val()
      .replace("\n", "<br>"); //개행처리
